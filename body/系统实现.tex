...
删除用户仅仅将用户从 user 表中删除，而不删除所有相关联的其余表中的 creatorId（或将其设置为 -1）。这样就能保证其他原先创建的考卷、题库、题目，不会受到其创建人的用户被删除，而出现异常情况。其 creatorId 的失效也仅仅影响了查询部分。\upcite{ref7,ref11,ref17}
